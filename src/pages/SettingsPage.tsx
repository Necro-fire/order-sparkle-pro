  ...
  return (
    <div className="space-y-6 max-w-2xl">
        ...
        <div className="flex gap-3">
          {colorEntries.map(([key, color]) => (
            <button
              ...
              className={cn(
                'w-10 h-10 rounded-md flex items-center justify-center transition-transform hover:scale-110',
                accentColor === key && 'ring-2 ring-offset-2 ring-offset-background'
              )}
              style={{
                backgroundColor: `hsl(${color.h}, ${color.s}, ${color.l})`,
                ...(accentColor === key ? { boxShadow: `0 0 0 2px hsl(${color.h}, ${color.s}, ${color.l})` } : {}),
              }}
              title={color.label}
            >
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">Em breve. Conecte o Lovable Cloud para gerenciar sua conta.</p>
      </motion.div>
    </div>
  );
}
