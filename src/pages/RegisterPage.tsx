...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <motion.div
        className="w-full max-w-sm surface-card rounded-lg p-8"
      ...
      >
        ...
        <h2 className="text-sm font-medium mb-1">Criar Conta</h2>
        <p className="text-xs text-muted-foreground mb-6">Registre-se para acessar o sistema.</p>

        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <Label className="text-xs text-muted-foreground">Nome</Label>
            <Input value={name} onChange={e => setName(e.target.value)} className="mt-1 bg-surface-2 border-0" style={{ boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.08)' }} placeholder="Seu nome" />
          </div>
          <div>
          ...
          <div>
            ...
            <Input type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="mt-1 bg-surface-2 border-0" style={{ boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.08)' }} placeholder="••••••••" />
          </div>
          <Button type="submit" className="w-full accent-glow">Criar Conta</Button>
        </form>
